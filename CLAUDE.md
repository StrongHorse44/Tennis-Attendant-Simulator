...
│   │   ├── CharacterAnimations.js # 29 procedural clips (CLIP_DEFS key poses → baked AnimationClips), Animator, contact probe
...
│   │   ├── OcclusionFade.js# Screen-door see-through (shared uniforms, patched into court materials at creation) for after-hours tennis
...
- 29 clips: `idle`, `idle_look`, `idle_shift`, `idle_watch`, `walk`, `run`, `talk`, `greet`, `wave`, `react_happy`, `react_annoyed`, `shrug`, `sit`, `clap`, `sit_clap`, `sit_wave`, `sit_cheer`, `drive`, `ready`, `split_step`, `shuffle_left`, `shuffle_right`, `forehand`, `backhand`, `serve`, `smash`, `volley_fh`, `volley_bh`, `pickup_ball`. The strokes' still racket-back poses (forehand / backhand 0.30, serve trophy 0.64, smash 0.32, volleys 0.16) are what hold-to-charge freezes on; keep a one-shot's timeScale below (duration − contact) / fadeOut or it ends before contact.
...
- **Entry**: the shift report's "Stay for a hit with Coach Rafa" button (`ShiftReport` `onTennis`), or talking to Rafa after `shiftClosingHour` (`TennisSession.offerFromNpc`, hooked in `Game._runAction` 'talk'). `begin()` sets 4:45 PM (`DAY_START`, a clear sky, weather rerolls held, the club clock frozen) and turns on `WeatherSystem.stadium`; the session's own clock then runs only while you play (not in the menu / results), at `DUSK_RATE[format]` (a short set reaches the floodlights in ~5 min of play, one set ~10, best of 3 ~20; drills use the short rate; capped at 9 PM), through golden hour and sunset until the floodlights take over. Saves taken meanwhile record the club's time (`clubTimeOfDay()`: clock-out), not the session's; takes Court 1 (`NPC.setAreaBusy`; every member match winds up, `TennisCrowd` sends the members home and seats the staff, `TennisOcclusion` fades whatever stands between the camera and the play), puts Rafa in the `playing` state (`npc.fullRateAnim`: full-rate animation and full-detail mesh at any camera distance) and hides the regular HUD (`body.cc-tennis`). While `tennis.active`, `Game._update` hands the frame to `TennisSession.update(dt)`, which steps physics, NPCs, matches, world, weather and its own camera. Leaving (`end()`): Next day if the report was already shown, else the clock goes to 19:00 and the normal clock-out shows the report. Nothing is saved mid-session; a reload returns to the report card. XP and the record are written when a drill / match ends (or is abandoned via Menu), then autosaved.
- **Files**: `TennisSession.js` (flow, player control, flights and events, scoring glue, XP, coach tips), `TennisBallSim.js` (analytic ball with per-segment gravity for spin: `SPIN` table — topspin dips and kicks, slice floats and skids; `BallPredictor` walks bounces; `planFlight` solves the flight time for a wanted net clearance; court constants), `TennisAI.js` (Rafa: intercept search on the predicted flight + contact probe, `DIFFICULTY` easy/medium/hard, shot choice, serves, drill feeds), `TennisScore.js` (pure scoring: deuce/ad, tiebreak at 6–6 / 4–4 with every-two-points serve rotation, change of ends; formats `short` / `set` / `bo3`), `TennisHUD.js`, `TennisCamera.js`, `TennisFX.js` (landing marker, aim guide, target rings), `TennisAudio.js` (pocks via `playBallHit`, net thud, racket whoosh, crickets; holds the daytime birdsong with `SoundSystem.ambientHold`), `TennisOcclusion.js` (see-through occluders, see Rendering → Courts), `TennisCrowd.js` (after hours: every member goes home via `NPC.setAway` — hidden, no physics body, `update()` skipped, and skipped by interactions, taps, the minimap, missions, matches and item props; staff Jess, Gus, Hank and Marcus walk to Court 1's benches / sideline, watch the ball and cheer through `react(kind, who, info)` with seated / standing clips, emoji, `npcs.json → courtside` lines and `SoundSystem.playApplause` / `playCheer` / `playCrowdVoice`; `end()` sends everyone home), `TennisCoach.js` (Coach Rafa's tips: rolling windows of swings, shots, serves and points, ~30 diagnosable patterns, one line per point at most — mostly between points — escalating hint → fix → root cause, notices improvement, changeover lines, first-time how-tos remembered in `courtcall.tennis.coach`, `summary()` for the results card; `opts.tips` off keeps only the essential how-tos).
- **Controls** (Top Spin-style hold-to-charge): move with the joystick / WASD (optional auto-move assist drifts toward the ideal hitting spot after a 0.22 s read). **Press** SWING (button, Space or J) as the ball comes to take the racket back — the stroke clip is held at its still backswing pose (`HOLD_T`; never freeze a clip with timeScale 0, `_holdPose` uses 1e-4) — **hold** to load power (0 → 1 over `CHARGE_FULL` 0.7 s, slower when tired, quicker "in the zone"), **release** to swing: the racket meets the ball `LEAD` (0.18 s) later. Holding past the ideal moment auto-releases late. While charging, the stick aims instead of moving you (Top Spin style) and your feet make adjustment steps toward the ideal hitting spot on their own — briskly with the assist, at 0.55× speed without it — so get there first, then load. A SWING held before Rafa strikes starts charging as his shot leaves the racket. Quality comes from the predicted ball–racket distance at contact (Perfect / Power shot! / Good / Early / Late / Stretch / Half volley! / whiff); the ring on the button closes at the ideal *release*, the fill inside it is the power. Five candidate strokes are evaluated at release (`STROKES`: forehand, backhand, `smash`, `volley_fh`, `volley_bh` — volleys only out of the air, the smash only on high balls out of the air, lobs, or from inside the court; `_strokeOk`). Power = pace but also risk (scatter grows above 0.6 power, more so with poor timing); a drop shot wants touch (power makes it long). Footwork: set feet (standing still ≥ 0.22 s) steady the shot, hitting on the run (> 3.2 m/s) costs pace and control; a split step as Rafa strikes gives a quicker first step; movement accelerates rather than snapping. Stick direction at contact aims (left/right, up = deep, down = short, short + wide = angle; early pulls cross-court, late pushes out); the optional aim guide shows the target while charging. Shots: Flat / Topspin / Slice / Lob / Drop (buttons or 1–5); a full-power topspin is heavier (`gMul` on the flight's gravity). **Serve**: press = the toss goes up (the clip holds the trophy pose at `SERVE_HOLD`), hold = power, release = swing, contact `SERVE_SWING` (0.3 s) later; the ideal release has the ball dropping back to the racket's full stretch (`TOSS_DROP` below the apex) — the toss meter's green zone. Early = hit on the rise (long), late = dropping (net); released before the toss or held until the ball falls = toss again (no fault); a swing at thin air = fault. Serve type from the selector: 1 flat, 2/4 kick, 3/5 slice. Faults, lets (net cord) and double faults. Shot scatter / pace / net margin also come from `PlayerProfile.getTennisStats()` (power, control, spin, speed, serve, stamina), fatigue, incoming pace / height (a high kicker, a low skidding slice), momentum and big-point pressure.
- **Momentum and pressure**: `session.momentum = [you, rafa]` (−1…1) — points (+0.06, winners / aces +0.06 more, long rallies +0.04), errors and double faults swing it; it decays ×0.85 per point. ≥ 0.7 is "In the zone!" (quicker charge, less scatter). The HUD shows yours minus Rafa's. `TennisScore.pressure()` → `{ kind: 'match' | 'set' | 'break', for }` announces big points and adds nerves (scatter) to the player facing them.
- **Rally challenge** drill (`rally`): Rafa plays cooperative topspin back (a little quicker every shot); three tries, stars from the best rally (`RALLY_STARS`). Matches show a live rally badge from 5 shots.
- **Balance** (headless bot, 50 ms release noise, hold ≈ 0.8 power, 3–4 short sets per difficulty): Easy ≈ 67–84 % of points to the bot, Medium ≈ 55–58 %, Hard ≈ 40–42 %; average rally ≈ 6 shots, longest 20–30; aces are rare both ways. Power habit on Medium (bot aiming away from Rafa): always tap ≈ 57 %, a controlled ~60 % load ≈ 71 %, always full ≈ 61 % with the most errors. Rafa reads the incoming ball: a soft ball gives him time (fewer mishits, more appetite to attack or approach), a heavy one rushes him (more mishits, softer, shorter replies). Tune in `DIFFICULTY` (`TennisAI.js`) and the sigma / margin lines in `_playerShot`. XP is capped at `XP_CAP` per stat per session.
...
- **Stadium floodlights** (`WeatherSystem.stadium` 0..1, after-hours tennis only): as the lamps come on at sunset the key light blends from the sun / moon to a bright near-white floodlight from almost overhead (`FLOOD_DIR`, `FLOOD_KEY` 2.4), the hemisphere fill and ambient lift (`FLOOD_HEMI`, `FLOOD_AMB`), and `EnvState.floodFactor` tells PostFX to lift most of the night grade and the court shader to step back its fake floodlight pool. The sky stays night. No lights are added (no new programs).
...
- **Courts** render their surface with one shader plane per court (zones, lines and clay dirt) — there is no line geometry. Hard courts that share a surround set `sharedPadLeft`/`sharedPadRight` in `map.json` to drop the inner edging. About 9 draw calls per court.
  - **See-through for the tennis camera** (`graphics/OcclusionFade.js`, driven by `tennis/TennisOcclusion.js`): the court chain-link is patched in place (it already discards via alphaTest); the opaque matte, metal, windscreen, sign and lamp-glass materials stay plain in the normal game (a shader that may `discard` loses early-Z on tile GPUs) and get precompiled "fade twins" (`courtOcclusionTwin(material)`, a hidden `courtOcclusionTwins` group so `_precompileShaders` compiles them) that `TennisOcclusion` swaps onto the meshes in its fade region for the session only — not the net, not the surface; the patch is `withOcclusionFade`, a 4×4 Bayer `discard` driven only by the shared `OCC_UNIFORMS` (a court frame, the half-space behind the camera-end baseline limited to |u| < 10.2 and above 0.28 m, plus camera→chest / camera→ball segments that stop short and never reach the net). No programs compile at session start and no draw calls are added; a fade only writes uniforms (plus the material swap at begin / end). `TennisOcclusion` also hides the funded scoreboard, collapses near tree instances that enter the view, drops near floodlight halos with `setDrawRange` (their `.visible` is reset every frame) and flags faded court meshes `userData.noAO` for GTAO; `end()` restores all of it.
...
| `courtcall.tennis` | `TennisSession` | After-hours tennis options (booleans): `assist`, `marker`, `aim`, `tips`, `changeEnds` |
| `courtcall.tennis.coach` | `TennisCoach` | Which first-time how-to lines Rafa has already given |
